    ...
    <div class="intro-y col-span-8 lg:col-span-6">
        ...
        <div class="intro-y box mt-5">
            ...
            <div id="transition-form" class="p-5">
                ...
                <div class="form-inline mt-3">
                    <label for="duration" class="form-label sm:w-30">triggerHook :</label>
                    <input id="duration" type="number" class="form-control" min="0" step="0.1" max="1"
                         v-model="ds.triggerHook" />


                <div @click="ds.triggerHook=0" class="ml-2 text-xs inline-flex items-center font-bold leading-sm uppercase px-3 py-1 rounded-full bg-white text-gray-700 border">
                    top
                </div>
                <div @click="ds.triggerHook=0.5" class="ml-2 text-xs inline-flex items-center font-bold leading-sm uppercase px-3 py-1 rounded-full bg-white text-gray-700 border">
                    middle
                </div>
                <div @click="ds.triggerHook=1" class="ml-2 text-xs inline-flex items-center font-bold leading-sm uppercase px-3 py-1 rounded-full bg-white text-gray-700 border">
                    bottom
                </div>
...
export default {
    ...
    methods: {
        loadData() {
            cmsService.getTypeNcode("main", "slide").then(res => {
        ...
        onSave() {
            console.log("데이터:::",this.ds)
            const content = JSON.stringify(this.ds)
            console.log("content::", content)
            CmsService.create("main", "slide", content).then(res => {
