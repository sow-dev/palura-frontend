<template>
<div>
  <p class="text-2xl font-semibold font-sans text-left">Most Popular</p>
  <Carousel :settings="settings" :breakpoints="breakpoints">
    <Slide  v-for="item in itemlist"
      v-bind:key="item.id" class="flex flex-col"
      >
      <ItemA v-if="item.order_state ===3" :id="item.id" :image="item.image" :title="item.title" :pen_name="item.pen_name"/>
    </Slide>
...
export default defineComponent({
  ...
  components: {
   ...
   created: function() {
        OrderService.dsplistStateBy("sold").then(
            res=>{
