    ...
    >
      ...
      >
        <table class="table table--sm table-report -mt-2">
          ...
          <tbody>
            <tr v-for="item in items" :key="item.id" class="intro-y h-20">
              ...
              <td class="text-center">
                ...
                <a
                  ...
                  href="javascript:;"
                  @click="onDeleteDanger(item)"
                >
                  Danger
                </a>
                <a
                  v-else-if="item.owner_wallet != item.real_wallet"
                  class="flex justify-center text-red-500 mt-1"
                  href="javascript:;"
                  @click="onModifyWarning(item)"
                >
                  Warning
                </a>
              </td>
        ...
        <!-- <Modal
          ...
          :deleteExec="deleteProduct"
        /> -->
        <Modal
          ref="Modal"
          @deleteExec="deleteProduct(delete_key)"
          @editExec="fixOwnerWallet(selectedItem)"
        />
      </div>
      <!-- END: Data List -->
      <!-- END: Pagination -->
...
export default {
  ...
  methods: {
    ...
    deleteProduct: function(deleteKey) {
      ProductService.delbyid(deleteKey)
        .then(() => {
          this.load();
          this.$refs.Modal.onSuccessModal(
            "Success",
    ...
    onDeleteProduct: function(id) {
      ...
      this.$refs.Modal.onDeleteModal(
        "Product Delete",
        "Do you really want to delete the product?"
      );
    },

    onModifyWarning: function(item) {
      this.selectedItem = item;

      this.$refs.Modal.onEditModal(
        "Minting error",
        `
      on DB: ${item.owner_wallet ? item.owner_wallet : "Null"} <br />
      on CHAIN: ${item.real_wallet ? item.real_wallet : "Null"} <br />
      Do you want to fix it?
      `
      );
    },

    onDeleteDanger: function(item) {
      this.delete_key = item.id;
      this.$refs.Modal.onDeleteModal(
        "Minting error",
        `
      on DB: ${item.owner_wallet ? item.owner_wallet : "Null"} <br />
    ...
    fixOwnerWallet: async function(item) {
