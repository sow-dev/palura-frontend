<template>
  <div class="intro-y box mt-3 p-5">
    <div class="view">
      ...
      <div
        v-else
        class="flex gap-5 pb-5 justify-center items-center w-full bg-white border-solid border-gray-300  hover:bg-blue-200 hover:text-white"
        @click="onModalProduct"
      >
        <!-- <h3 :src="getInfo.name" class="imgMain" /> -->

        <div
          v-if="getInfo.sub_content_type === 'MP4'"
          class="flex justify-center"
        >
          <video
            :src="getInfo.thumbnail"
            class="w-36 h-36"
            autoplay
            loop
          ></video>
          <!-- <button class="btn btn-primary w-full"  > change Product</button> -->
        </div>
        <div v-else>
          <img :src="getInfo.image" class="cursor-pointer h-36" />
        </div>
      ...
      <div class="box p-5 mt-3">
        ...
        <div v-if="getInfo.image">
          ...
          <ContentViwer
            v-model="getInfo.image"
            :fileType="getInfo.main_content_type"
          />
        </div>
        <div v-else>
          <input
        ...
        <div v-if="getInfo.thumbnail">
          ...
          <ContentViwer
            v-model="getInfo.thumbnail"
            :fileType="getInfo.sub_content_type"
          />
        </div>
        <div v-else>
          <input
...
export default {
  ...
  data: function() {
    return {
      ...
      thumbnail: "",
      main_content_id: 0,
      main_content_type: "jpg",
      sub_content_id: 0,
      sub_content_type: "jpg",
      tag: "",
      tag_list: [],
      owner_id: "",
      chain_id: "",
  ...
  methods: {
    ...
    onSelectProduct: async function(item) {
      ...
      this.main_content_id = this.getInfo.main_content_id;
      this.main_content_type = this.getInfo.main_content_type;
      this.sub_content_id = this.getInfo.sub_content_id;
      this.sub_content_type = this.getInfo.sub_content_type;
    ...
    createOrder: function(ret) {
      ...
      const main_content_id = this.main_content_id;
      const sub_content_id = this.sub_content_id;
      const main_content_type = this.main_content_type;
      const sub_content_type = this.sub_content_type;
